...
def market_page():
    ...
    if request.method == "POST":
        ...
        if s_item_object:
            # check if the user really owns the item
    ...
    if request.method == "GET":
        ...
        # Get owned items and display them on the page
...
def register_page():
    form = RegisterForm()
    # Verify when the user clicks the submit button
    # Validations to check for certain requirements
    if form.validate_on_submit():
        ...
        # Direct user to the market page after registration
        return redirect(url_for('market_page'))
    if form.errors != {}:
...
def login_page():
    form = LoginForm()
    if form.validate_on_submit():
        # check if the user exists and whether the password is correct
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash("Username and password do not math! Please try again", category="danger")
